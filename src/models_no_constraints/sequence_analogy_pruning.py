...
def main():
    ...
    parser.add_argument("--nb-test-analogies", dest="nb_test_analogies", type=int, help="Number of test analogies", required=False, default=20)
    parser.add_argument("--nb-keeping-in-test", dest="nb_keeping_in_test", type=int, help="Number of keeping test analogies", required=False, default=20)
    parser.add_argument("--nb-pruning-in-test", dest="nb_pruning_in_test", type=int, help="Number of pruning test analogies", required=False, default=20)
    ...
    parser.add_argument("--sequence-length", dest="sequence_length", help="Length of sequences of the decisions", required=False, default=4, type=int)
    parser.add_argument("--padding", help="Zero padding mode", required=False, default="between", choices=["before", "between", "after"])
    ...
    parser.add_argument("--analogy-batch", dest="analogy_batch", required=False, default=10_000, type=int)
        ...
        if qid_emb is not None:
            ...
            if q_adjacency is not None and ("P31" in q_adjacency["claims"] or "P279" in q_adjacency["claims"] or
                ...
                for cl in classes:
                    cl_emb = utils.get_hashmap_content(utils.WIKIDATA_PREFIX + cl + '>', embedding_hashmap)
                    if cl_emb is not None:
                        sequence = [numpy.transpose(numpy.array([qid_emb]), (1, 0)), numpy.transpose(numpy.array([cl_emb]), (1, 0))]
                        sequenced_decisions[qid][cl] = sequence
                    else:
                        logger.debug(f"Class {cl} not found in embeddings")
                ...
                while classes:
                    ...
                    logger.info(f"{qid} : depth = {depth}, {len(classes)} entities to classify.")
                                ...
                                if utils.get_node_degree(cl_adjacency) < args.node_degree:
                                    ...
                                    for sub_cl in sub_classes:
                                        ...
                                        if sub_emb is not None:
                                            sequence = sequenced_decisions[qid][cl] + [numpy.transpose(numpy.array([sub_emb]), (1, 0))]
                                            sequenced_decisions[qid][sub_cl] = sequence
