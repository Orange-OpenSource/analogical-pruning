...
def main():
    ...
    parser = argparse.ArgumentParser(prog="up_expansion", description="Expand QIDs along the ontology "
                                                                      "hierarchy of Wikidata from a set of "
                                                                      "seed QIDs -- only up -- no pruning")
    ...
    for q in tqdm.tqdm(qids_to_expand):
        ...
        if q_adjacency is not None and ("P31" in q_adjacency["claims"] or "P279" in q_adjacency["claims"]):
            classes = set()
                ...
                for cl in classes:
                    ...
                    if cl_adjacency is not None and "claims" in cl_adjacency:
                        if "P279" in cl_adjacency["claims"]:
                            cl_superclasses = get_relation_objects(cl_adjacency["claims"]["P279"])
                            new_classes |= (cl_superclasses - seen_classes)
