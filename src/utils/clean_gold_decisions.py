...
def main():
    ...
    confidence = dict()
    for _, r in decisions[(decisions["confidence"].notna())].iterrows():
        if r["from"] not in confidence:
            confidence[r["from"]] = dict()

        if r["QID"] not in confidence[r["from"]]:
            confidence[r["from"]][r["QID"]] = r["confidence"]
        elif confidence[r["from"]][r["QID"]] != r["confidence"]:
            logger.error(f"Contradictory confidence for {r['from']} and {r['QID']}")

    decisions.drop("confidence", axis=1)
    ...
    for q in tqdm.tqdm(starting_qids):
        ...
        if q_adjacency is not None and ("P31" in q_adjacency["claims"] or "P279" in q_adjacency["claims"] or
            ...
            while classes:
                ...
                for cl in classes:
                    if q not in confidence:
                        confidence[q] = dict()

                    if cl not in confidence[q]:
                        confidence[q][cl] = ""
    ...
    output_decisions = pandas.DataFrame(output_decisions, columns=["QID", "label", "from", "starting label", "target",
                                                                   "confidence"])
